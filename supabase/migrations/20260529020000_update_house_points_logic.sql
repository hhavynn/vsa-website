-- Update House standings to count qualifying attendance instead of point sums.
-- One member attending one qualifying event = 1 House point.
-- 2025-2026 House points start on/after 2025-11-08 (House Reveal).
...
select
  ...
  t.academic_year_end,
  -- Formula: Count member-event check-ins where points were earned
...
where e.date is not null
  ...
  -- Exclude events with 0 points (required duties, etc)
  and mea.points_earned > 0
  -- Enforce 2025 House Reveal start date
...
select
  ...
  t.academic_year_start,
  t.academic_year_end,
  -- Total house points for this event = member count
  count(*)::int as total_points,
...
select
  ...
  t.academic_year_start,
  t.academic_year_end,
  -- Shows how many House points this member contributed (attendance count)
  count(*)::int as total_points,
  count(*)::int as events_attended,
  count(distinct mea.event_id)::int as unique_events,
  max(e.date) as latest_activity_at
