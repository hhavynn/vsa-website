...
-- This migration restores 2025-2026 Mario House assets after legacy seeding removed current-year rows. 
-- It preserves existing media URLs on conflict.

do $$
begin
  -- 1. Ensure 2025-2026 Houses exist with consistent lowercase house_key and correct labels.
  -- We use predictable lowercase 'house_key' for slugging and matching.
  ...
  -- Donkey Kong
  insert into public.house_page_assets (academic_year_start, academic_year_end, house, house_key, display_name, description, accent_color, image_url, emoji, display_order, is_active)
  values (2025, 2026, 'Donkey Kong', 'donkey-kong', 'Donkey Kong', 'Loud, wild, and impossible to ignore.', '#ca8a04', '/images/houses/2025_donkey-kong.webp', '🦍', 1, true)
  on conflict (academic_year_start, house) do update
  set
  ...
  -- Boo
  insert into public.house_page_assets (academic_year_start, academic_year_end, house, house_key, display_name, description, accent_color, image_url, emoji, display_order, is_active)
  values (2025, 2026, 'Boo', 'boo', 'Boo', 'Silent until it is time to go off.', '#94a3b8', '/images/houses/2025_boo.webp', '👻', 2, true)
  on conflict (academic_year_start, house) do update
  set
    house_key = 'boo',
    display_name = 'Boo',
  ...
  -- 2. Secondary cleanup: Ensure no duplicates with incorrect casing or name variants if they exist
