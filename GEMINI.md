...
- **Playbooks** (`.claude/agents/<name>.md`) are source-of-truth **domain personas** — distinct from skills. Infer the matching playbook automatically. If native delegation is unavailable, execute bounded playbook passes sequentially while preserving separate implementation and review roles (workflow §§5 and 9). Never claim a Claude-native subagent was invoked. The canonical roster is `.claude/agents/README.md`; do not maintain a second copy here.
...
- **Superpowers** methodology is routed proportionally in workflow §6; follow that mapping rather than applying every method to every task.
...
- Verification ownership is defined once in workflow §13 and `.claude/skills/vsa-validation-and-qa/SKILL.md`. Run only checks that answer a new question, then report exact evidence; do not maintain a separate command list here.
