...
| Event interest | Buttons; `event_interest_counts` and `localStorage` | Public aggregate estimate/device-local choice; the server does not verify the prior choice | Explain local state; no attributable server export | Clear local key; no individual server deletion | No user ID stored; count adjustments are approximate |
