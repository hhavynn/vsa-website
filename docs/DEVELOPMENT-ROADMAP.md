...
- **`secure-ai` Edge Function** — resolved by [#353](https://github.com/hhavynn/vsa-website/issues/353): repo source was removed after source search found no callers. Production was verified on 2026-08-05 — the function was never deployed, and `OPENAI_API_KEY` has been unset from the project's secrets. The exposed key still needs revoking in the OpenAI dashboard, which keeps #353 open.
