...
2. **CI runs tests but does not gate anything.** `main` has **no branch protection** (verified 2026-07-06: `gh api repos/hhavynn/vsa-website/branches/main/protection` → HTTP 404 "Branch not protected"). This is why `AGENTS.md` line 246 says "There is no automated CI test gate" even though deploy.yml runs tests — the jobs run, but a failing run blocks neither merges nor the push that already landed. Run `npm run lint && npm run build` and `CI=true npm test -- --watchAll=false` locally before pushing; nothing will save you server-side.
