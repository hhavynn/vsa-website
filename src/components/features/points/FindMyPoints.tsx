...
export function FindMyPoints({ variant = 'panel', className = '', showHeader = true }: FindMyPointsProps) {
  ...
  const academicYears = useMemo<AcademicYearOption[]>(() => {
    ...
    yearsWithData.forEach((year) => {
      ...
      if (existing) {
        ...
        years.set(year, { year, label: `${year}-${year + 1}`, isActive: false, hasData: true });
      }
    });
  ...
  const initialSelectedYear: SelectedYear | null = defaultYearReady ? resolvedDefaultYear ?? 'all' : null;

  useEffect(() => {
    if (hasUserSelectedYear || selectedYear !== null || initialSelectedYear === null) return;
  ...
  const { data: entries = [], isLoading: entriesLoading, isFetching, error } = useFindMyPoints(selectedYear);
  ...
  const selectedYearLabel = useMemo(() => {
    ...
    return academicYears.find((y) => y.year === selectedYear)?.label || `${selectedYear}-${selectedYear + 1}`;
  ...
  const showNoResults = trimmedQuery.length > 0 && matches.length === 0 && !loadingState && !initializing;
  ...
  return (
    <section className={containerClass} aria-labelledby="find-my-points-heading">
        ...
        <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <StickerBadge rotation={-2} color="accent">FIND MY POINTS</StickerBadge>
              {selectedYearLabel && (
                <StickerBadge rotation={1} color="primary" size="sm">{selectedYearLabel}</StickerBadge>
              )}
            </div>
            <h2 id="find-my-points-heading" className={variant === 'page' ? 'vsa-section-title' : 'font-serif text-2xl font-bold leading-tight'} style={{ color: 'var(--text)' }}>
              Look up your points
            </h2>
          ...
          <div className="shrink-0">
            <label className="block font-mono text-[10px] font-bold uppercase tracking-wider opacity-60" style={{ color: 'var(--text3)' }}>
              Year
            </label>
            <select
              value={selectedYear ?? ''}
      ...
      <div className="relative">
        ...
        <Input
          ...
          placeholder="Type your name (e.g. Jane Nguyen)"
          value={query}
          onChange={(e) => {
      ...
      <div className="mt-6">
        {error ? (
          <div className="scrapbook-empty text-sm" style={{ color: 'var(--text3)' }}>
          ...
          <div className="scrapbook-empty">
            <p className="font-sans text-sm" style={{ color: 'var(--text3)' }}>
              No member activity recorded for {selectedYearLabel} yet. Try the <strong>All-Time</strong> view above.
            </p>
          </div>
...
function NoResultsCard({ query, yearLabel }: { query: string; yearLabel: string }) {
  return (
    <div className="rounded-xl border-2 border-[var(--border)] bg-[var(--surface2)] p-5">
      <p className="font-serif text-base font-bold" style={{ color: 'var(--text)' }}>
        No match for "{query}" in {yearLabel}.
      </p>
      <p className="mt-2 font-sans text-sm" style={{ color: 'var(--text2)' }}>
        Try a shorter spelling, switch the academic year, or check the All-Time view. If you attended events but don't see yourself, you can request a correction below.
      </p>
      <div className="mt-4">
        <Link to={CORRECTION_HREF} className="vsa-btn-primary inline-flex whitespace-nowrap">
  ...
  return (
    <div>
      <div className="mb-3 flex items-baseline justify-between gap-3">
        ...
        <p className="font-mono text-[10px] font-bold uppercase tracking-wider" style={{ color: 'var(--text3)' }}>
          {yearLabel}
        </p>
      </div>
      <p className="mb-4 font-sans text-xs" style={{ color: 'var(--text3)' }}>
        Pick the one that's you. We don't show emails — use class year or college to tell yourself apart.
      </p>
          ...
          <li key={entry.member_id}>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="truncate font-serif text-[15px] font-bold" style={{ color: 'var(--text)' }}>
                  {entry.full_name || 'VSA Member'}
                </div>
                <div className="truncate font-sans text-[11px]" style={{ color: 'var(--text3)' }}>
                  {[entry.graduation_year, entry.college].filter(Boolean).join(' • ') || 'VSA Member'}
                </div>
              </div>
              <div className="shrink-0 text-right">
                <div className="font-mono text-[9px] font-bold uppercase tracking-wider opacity-60">RANK</div>
                <div className="font-mono text-base font-black" style={{ color: 'var(--text)' }}>#{entry.rank}</div>
              </div>
            </button>
          </li>
        ))}
...
function TrustNote() {
  return (
    <div className="mt-5 flex items-start gap-3 rounded-lg border border-[var(--border)] bg-[var(--surface2)]/60 px-4 py-3">
      ...
      <div className="font-sans text-[11px] leading-[1.6]" style={{ color: 'var(--text3)' }}>
        Points refresh after admin imports — give it 24–48 hours after an event. Yearly totals only count events assigned to an academic term (Fall / Winter / Spring).
      </div>
    </div>
  );
}
