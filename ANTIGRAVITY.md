...
- **Playbooks** (`.claude/agents/<name>.md`) are source-of-truth **domain personas** — distinct from skills. Read the matching playbook and apply it using Antigravity's own agent/delegation mechanisms. **Never claim a Claude-native subagent was invoked** — you are reading the file as a playbook, not running a Claude subagent.
...
- **Superpowers** methodology is summarized in the workflow doc §6 (the plugin is installed for Claude; Antigravity uses the encoded lifecycle). Use it for meaningful work; skip the ceremony for tiny tasks.
